...
async function listHermesSessionSummariesAllProfiles(source?: string, perProfileLimit = 2000): Promise<any[]> {
  ...
  for (const profile of profiles) {
    const rows = await listHermesSessionSummaries(source, perProfileLimit, profile)
...
async function listHermesSessionSummaryGroupsAllProfiles(limitPerSource = 20, includedSessionIds: string[] = []): Promise<any> {
  ...
  const profiles = listProfileNamesFromDisk()
  for (const profile of profiles) {
    const result = await listHermesSessionSummaryGroups(limitPerSource, profile, includedSessionIds)
...
export async function listHermesSessions(ctx: any) {
  ...
  const allSessions: any[] = isHermesAgentAvailable()
    ? (allProfiles
...
export async function listHermesSessionGroups(ctx: any) {
  ...
  const [hermesResult, localSessions] = await Promise.all([
    isHermesAgentAvailable()
      ? (allProfiles
        ? await listHermesSessionSummaryGroupsAllProfiles(limit, includedIds)
  ...
  const hermesGroups = new Map<string, { source: string; sessions: any[]; hasMore: boolean }>(
    (hermesResult as any).groups.map((group: any) => [group.source, group]),
  )
  const sources = new Set<string>([
    ...hermesGroups.keys(),
    ...localSessions.map((session: any) => String(session.source)).filter(Boolean),
  ])
...
export async function remove(ctx: any) {
  ...
  if (!existing) {
    ...
    for (const p of profiles) {
      try {
        const row = await getExactHermesSessionDetailForProfile(sessionId, p)
        if (row) {
          foundProfiles.push(p)
          await deleteHermesSessionForProfile(sessionId, p)
        }
...
export async function getConversationMessagesPaginated(ctx: any) {
  ...
  try {
    const { SessionMessageSync } = await import('../services/session-message-sync')
