...
import { pollNousLogin, startNousLogin } from '@/api/hermes/nous-auth'
...
const {
  ...
  approvedMessage: () => t('models.nousApproved'),
...
async function startLogin() {
  ...
  try {
    ...
    startPolling({
      intervalMs: 3_000,
      poll: async id => {
        const result = await pollNousLogin(id)
        ...
        if (result.status === 'denied') return { kind: 'failed', message: t('models.nousDenied') }
