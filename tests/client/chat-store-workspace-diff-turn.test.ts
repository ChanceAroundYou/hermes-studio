...
describe('chat workspace diff turn association', () => {
  ...
  it('merges workspace changes only when their older message page is loaded', async () => {
    const fullMessages = [...chatApi.resumePayload.messages]
    chatApi.resumePayload = {
      ...chatApi.resumePayload,
    ...
    // REST-first initial fetch (offset 0) should not clobber the paginated resume payload.
    // Simulate a transient failure so restLoadedMessages stays false and resume wins.
    // The subsequent loadOlderMessages(offset=2) then fetches the older page.
    sessionApi.fetchSessionMessagesPage.mockImplementation(async (sid: string, offset: number) => {
      if (offset === 0) throw new Error('transient REST failure for test')
      return olderPage as any
    })
